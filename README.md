...
RokidのHUD（文字・アイコン）は緑1色ですが、純正「カメラ」を開くと、「ライブ映像」と「背景なし（省電力）」のどちらでもWindows表示がフルカラーの撮影画面へ自動で切り替わります。カメラを閉じると、選んでいた表示モードへ自動で戻ります。

<table>
  <thead>
    <tr>
      <th width="24%">撮影方法</th>
      <th width="28%">ライブ映像</th>
      <th width="28%">背景なし（省電力）</th>
...
純正カメラのカラー画面を見ながら撮る、テンプルのボタンですぐ撮る、という2つの使い方を選べます。
...
- **Rokidの開発用5ピンケーブル**
  RV101付属の3ピンケーブルは充電専用です。Windowsとの初回接続には使えません。開発用ケーブルはAmazonなどで入手可能です。
